...
def preprocess_image(image_path, target_size=(260, 260)):  # Thay đổi kích thước này
...
def build_model():
    base_model = EfficientNetB2(  # Thay đổi này
        weights="imagenet", include_top=False, input_shape=(260, 260, 3)  # Thay đổi này
    )
...
for fold_no, (train_indices, test_indices) in enumerate(
    ...
    metrics_logger = MetricsLogger(
        f"metrics_EfficientNetB2_v1_tangcuong_fold_{fold_no}.log",
        X_val,
        y_val,
        fold_no,
        f"confusion_matrix_EfficientNetB2_v1_tangcuong",
    )
    ...
    save_classification_report(
        ...
        y_pred,
        class_names,
        f"classification_report_EfficientNetB2_v1_tangcuong.txt",
    )
